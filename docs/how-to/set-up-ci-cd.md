...
| `config/environments.yaml` | Single config: governed objects + per-environment deploy target (`database` / `warehouse` / optional `role`) |
...
**target** (`database`, `warehouse`, optional `role`). Object schemas come from
each object's own FQN, so there are no separate `*_schema` keys:
...
`prod.database` (schema preserved from each FQN). Use `--dry-run` to print the
resolved target and the objects that would be deployed without touching
Snowflake.
